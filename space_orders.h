...
#pragma omp simd
      ...
      {
        //printf(" seq Visiting : u[%d][%d][%d]\n", titer,xi,yi);
        grid[t1][xi][yi] = (grid[t0][xi][yi] + grid[t0][xi - 1][yi] + grid[t0][xi + 1][yi] + grid[t0][xi][yi - 1] + grid[t0][xi][yi + 1] +
                                               grid[t0][xi - 2][yi] + grid[t0][xi + 2][yi] + grid[t0][xi][yi - 2] + grid[t0][xi][yi + 2])/9; }
    }
  }
  return grid;
}
...
double ***so2_tiled_skewed_jacobi_3d(int timesteps, int nrows, int ncols, double ***grid, int omp_opt, int tile_size)
{
  ...
  // x blocking
  for (int xblk = x_m; xblk < x_M + 2*timesteps - 1; xblk += x_blk_size)
  {
    // t interchange
    for (int titer = 0; titer < 2*timesteps - 1; titer+=2)
    {
      t1 = !t1;
      t0 = !t1;
...
#pragma omp simd
        ...
        {
          //printf(" Visiting : u[%d][%d][%d]\n", titer,xi-titer,yi-titer);
          ...
          grid[t1][xi - titer][yi - titer] = (grid[t0][xi - titer][yi - titer] + grid[t0][(xi - titer) - 1][yi - titer] + grid[t0][(xi - titer) + 1][yi - titer] + grid[t0][xi - titer][yi - titer - 1] + grid[t0][xi - titer][yi - titer + 1] + 
                                                                                 grid[t0][xi - titer - 2][yi - titer] + grid[t0][xi - titer + 2][yi - titer] + grid[t0][xi - titer][yi - titer - 2] + grid[t0][xi - titer][yi - titer + 2])/9;}
      }
    }
  }
  return grid;
